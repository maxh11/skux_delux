...
class MinimaxPlayer:
    ...
    def action(self):
        ...
        # Mid game, use alpha beta search
    ...
    def update(self, colour, action):
        ...
        # This is the code used for TD leaf lambda with lambda = 0.
        ...
        if is_game_over(self.current_node.state):
            ...
            for i in range(len(num_pieces_evs)):
                sum1 += num_pieces_evs[i] * (abs(math.tanh(eval_evs[i])) - sn)
            w1new = w1 - 0.01 * sum1
            print(w1new)
            parser.set('weights', 'w1', str(w1new))

            for i in range(len(manhattan_dist_evs)):
                sum2 += manhattan_dist_evs[i] * (abs(math.tanh(eval_evs[i])) - sn)
            w2new = w2 - 0.01 * sum2
            print(w2new)
            parser.set('weights', 'w2', str(w2new))

            for i in range(len(num_groups_evs)):
                sum3 += num_groups_evs[i] * (abs(math.tanh(eval_evs[i])) - sn)
            w3new = w3 - 0.01 * sum3
            print(w3new)
