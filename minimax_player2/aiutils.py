...
class State:
    ...
    def evaluation(self, colour=WHITE):
        ...
        eval += w1 * ((0.90 ** self.total_pieces()) * (self.total_pieces(colour) - self.total_pieces(opponent(colour))))
...
def heuristic(colour, state):
    ...
    if colour == BLACK:
        ...
        # distance, **higer** is better
        # return state.eval(colour)
        return state.total_black() - state.total_white() - abs(math.tanh(manhattan_dist(state,
